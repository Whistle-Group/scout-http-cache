...
exports.getCachedValue = function getCachedValue(key, cacheOptions) {
  var cache;
  cacheOptions = hoek.applyToDefaults(DEFAULT_CACHE_OPTIONS, cacheOptions || {});
  ...
  return new Promise(function (resolve, reject) {
    cache.get(key, function (err, value, cached, report) {
      ...
      if (err) { return reject(err); }  // fallback to HTTP request on error
      if (!value) { return reject(new Error()); }  // fallback to HTTP request on no data found

      this.cachedValue = value;
      valueAgeMillis   = Number(new Date()) - cached.stored;
      needsRefresh     = cacheOptions.refreshAfter && valueAgeMillis > cacheOptions.refreshAfter;

      if (needsRefresh) { return reject(new Error()); }  // fallback to HTTP request when data needs refreshing
