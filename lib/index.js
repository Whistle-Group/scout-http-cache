...
exports.request = function get(options, callback) {
  var url, method, cacheKey, httpClient, httpOptions, cacheOptions, cachedValue;
  ...
  cacheKey     = crypto.createHash('sha1').update(['get', url, httpOptions.body].join('_')).digest('hex');

  return cache.getCachedValue(cacheKey, cacheOptions)
